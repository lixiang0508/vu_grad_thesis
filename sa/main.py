...
def visualize(obstacle_path, terminal_path):
    ...
    if alter_opt_chro.cost < opt_chro.cost:
        ...
        outer_vis(alter_opt_chro, softobs, hardobs, obstacle_path[22:25] + 'png')
        ...
        outer_vis(opt_chro, softobs, hardobs, obstacle_path[22:25] + 'png')
...
'''
# T is the number of iterations, alpha is the cooling parameter, Tf is the final temperature
# iter is the number of internal iterations
def Metrospolis(f, f_new , T):  # Metropolis准则 #todo change this principle
    if f_new <= f:
        return 1
    else:
        ...
        if random.random() < p:
            ...
            return 0
'''
def accept_probability(new_solution, current_solution, temperature):
...
def sa_function(obstacle_path, terminal_path, T = 3, Tf = 0.01, alpha = 0.98, iter = 200):
    chromosomes = []
    # Process the file and print the results
    obstacles = process_obstacle_file(obstacle_path)
    print('obstacles,', obstacles)
    ...
    for i in range(50):
        chromosomes.append(Chromosome([], ''.join(random.choice('01') for _ in range(k)), terminals, all_obstacles))
    print('After init 3 we have how many chromosomes ', len(chromosomes))
    min_cost_chromosome = min(chromosomes, key=lambda x: x.cost)


    print("Chromosome with the minimum cost has a cost of:", min_cost_chromosome.cost) #This one is the initial chromosome
    current_solution = min_cost_chromosome
    current_temp = T
    pmax = 0.99
    pmin = 0.60
    while current_temp > Tf:
        # inner loop
        # 选取flipMove, add, remove, replace的概率和原来类似 add, remove, replace各占剩余的三分之一
        for i in range(1, iter+1):
            p_flipMove = max(pmax * (1 - i / 1000), pmin)
            p_add = (1 - p_flipMove) / 3
            p_remove = (1 - p_flipMove) / 3
            p_replace = (1 - p_flipMove) / 3

            probs = [p_flipMove, p_add, p_remove,p_replace]
            chosen_index = np.random.choice([0, 1, 2,3], p=probs)  # 0 1 2 3
            if chosen_index == 0:
                # todo if we happen to have no obstacles, there must be some inti 3 instances (no steiner points)
                # in this case we cannot flipMove
                new_solution = current_solution.flipMove(avg_dis, i)
            elif chosen_index == 1:
                new_solution = current_solution.add_steiner_mutation()

            elif chosen_index == 2:
                new_solution = current_solution.remove_steiner_mutation()
            else:
                new_solution = current_solution.replace_steier_mutation()

            if new_solution.cost < current_solution.cost:
                current_solution = new_solution
            else:
                # accept the new solution with a certain probability
                if random.random() < accept_probability(new_solution, current_solution, current_temp):
                    current_solution = new_solution
        print('current optimal cost is',current_solution.cost)
        current_temp *= alpha
    outer_vis(current_solution, softobs, hardobs, obstacle_path[22:25] + 'png')
    print('nodes', current_solution.get_nodes())
    print('steiner points', current_solution.steinerpts)
    print('mst', current_solution.mst)
    print('bins', current_solution.bins)
...
if __name__ == '__main__':
    ...
    results = {}
    #for i in range(1, 24):
    #[61, 101, 301, 401, 501, 1001]
    for i in range(1, 12, 1):
        obstacle_path = 'SolidObstacles/added_obstacles/obstacles' + str(i) + '.csv'
        terminal_path = 'solid_terminals/added_terminals/terminals' + str(i) + '.csv'
        # for i in range(10):
        obs = 'soft_obs_' + str(i)
        chro = sa_function(obstacle_path, terminal_path)
        results[obs] = chro.cost
        print(obstacle_path, ' ', results[obs])
        with open('output_sa.txt', 'a') as file:
            # 将浮点数转换为字符串并写入文件
